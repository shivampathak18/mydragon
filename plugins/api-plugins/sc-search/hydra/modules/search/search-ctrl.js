...
//var ddg = require('ddg'); 
var nlpController = require('./modules/nlp-engine/nlp-ctrl'); 
var opController = require('./modules/operation-engine/op-ctrl'); 
/**
* Search happens here
*
* @param req
...
exports.search = function(request, reply) {
	var searchResponse= {};
	//var nlp = nlpController.main(request,reply);

	Q.allSettled([_this.searchES(request,reply),nlpController.natural(request,reply)])
	.then(function (results) {
		results.forEach(function (result,index) {
			...
			if (result.state === "fulfilled") {
				...
				//if(index === 0)
					//console.log('concating results');
					//console.log(JSON.stringify(result.value));				
					
				/*if(index === 1)
					searchResponse['nlpResult']= result.value;
				else*/
					searchResponse = _.extend(searchResponse, result.value);
				/*if(index == 0){
				...
				searchResponse = _.extend(searchResponse, {});
...
exports.natural = function(request, reply){
	SearchLib.get_nlp_query(request.params.queryString,
		function(err, result){
			if(err){
	...
	*/
	if (!(_.isEmpty(options.parent_path))) {
		...
		.then(function(response) {
			...
			searchResponse = SearchLib.includeFilterDetails(response,searchResponse);
			console.log('include filters successful');
